...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError(
        'The user to who this token belongs does not longer exists.',
        401
      )
    );
  }
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(
        'The user has changed his password recently. Please log in again.',
        401
      )
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    // 3) Send the email with token to user
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save(); // don't have to turn off the validators
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // 4) Log the user in, send new JWT to the client
  createSendToken(user, 200, res);
});
